...
local Settings = {
    ...
    FollowStrength = 0.7,  -- How strongly to follow the fish (0.0-1.0)
    SmoothingFactor = 0.8,  -- How smooth the following movement is (0.1-1.0)
}
        ...
        local ok, err = pcall(function()
            while Settings.AutoReel and myVersion == reelVersion do
                ...
                if playerbar and fish then
                    ...
                    -- NEW APPROACH: Directly manipulate the control bar position
                    -- Calculate the target position for the player bar
                    local targetPosition = predictedFishX
                    
                    -- Apply smoothing to make movement more natural
                    local smoothingFactor = 0.8  -- Higher values = more responsive, lower = smoother
                    local newPosition = playerCenterX + (targetPosition - playerCenterX) * smoothingFactor
                    
                    -- Calculate how much we need to move the bar
                    local moveAmount = newPosition - playerCenterX
                    
                    -- Determine if we need to "hold" or "release" based on movement direction
                    if math.abs(moveAmount) > 2 then  -- Only move if difference is significant
                        if isrbxactive() then
                            if moveAmount > 0 then
                                -- Move right - simulate holding
                                if not isHolding then 
                                    pcall(mouse1press); 
                                    isHolding = true 
                                end
                                -- Brief hold to move the bar
                                task.wait(0.01)
                                pcall(mouse1release)
                                isHolding = false
                            else
                                -- Move left - simulate brief hold
                                pcall(mouse1press)
                                task.wait(0.01)
                                pcall(mouse1release)
                            end
